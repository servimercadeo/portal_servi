...
class PayingsController extends Controller
{
    ...
    public function indexLiquidaciones(){
        ...
        if(Auth::user()->hasRole(['admin','dtv_hugs'])){
            $payinds = $query->paginate(30)->orderBy('periodo');
        ...
        $payinds = $query->where('id_pais', Auth::user()->id_pais)->paginate(30)->orderBy('periodo');
    ...
    public function fillter(Request $request){
        ...
        $payings = Paying::where($fill)->with('pais:id,name')
                            ...
                            ->where('tipo', $tipo)->paginate(30)->orderBy('periodo');
